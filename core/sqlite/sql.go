...
func InsertFilesDatabase(db *sqlites.SqliteDB, assetID string, totalPieces int64, operates, status int, modTime time.Time) error {
...
func UpdateFileDatabaseStatus(db *sqlites.SqliteDB, assetID string, operates, status int) error {
	data := map[string]interface{}{
		"status": status,
	}
...
func InsertSlicesDatabase(db *sqlites.SqliteDB, assetID, sliceHash string, current, status int) error {
	data := map[string]interface{}{
		"assetID":    assetID,   // 文件资产的唯一标识
...
func UpdateSlicesDatabaseStatus(db *sqlites.SqliteDB, assetID, sliceHash string, status int) error {
	data := map[string]interface{}{
		"status": status, // 状态
	}
...
func SelectOneSlicesDatabaseStatus(db *sqlites.SqliteDB, assetID string) (*struct {
...
func SelectOneAssetID(db *sqlites.SqliteDB, assetID string) bool {
	columns := []string{
		"assetID", // 文件资产的唯一标识
	}
	conditions := []string{"assetID=?"} // 查询条件
